...
function [ vignetteImg ] = EllipticalVignette(inputImage, scaleLevel)
    if scaleLevel < 0
    ...
    maxRadius = sqrt(offsetX ^ 2 + (16 * offsetY ^ 2) / 7);
    for x = 1:imageInfo.Width
        for y = 1:imageInfo.Height
            radius = sqrt((x - vignetteCenterX) ^ 2 + (16 * (y - vignetteCenterY) ^ 2 / 7));
