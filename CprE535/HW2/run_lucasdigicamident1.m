...
inPath = 'FF';      % folder containing input images

outPath = 'out';      % This is where the prnu reference data are saved (.mat file)
...
switch pmode
    case 'averagePRNU'
        ...
        status = lucasdigicamident(opMode, inPath, outPath, prnuNameID, iList, addargs);
    case 'singlePRNU'
        for i=1:nL
            prnuNameID = {[prnuName{i}{:} '_L']};
            iListID = iList(i);
            status = lucasdigicamident(opMode, inPath, outPath, prnuNameID, iListID, addargs);
        end
end
...
if exist('rListFile') == 1
    ...
    for i = 1:nList
        ...
        [status, ref1, ref2, colrNames, corr] = lucasdigicamident(opMode, inPath, outPath, prnuName);
    ...
    status = lucasdigicamident(opMode, inPath, outPath, prnuName);
