/*
	Acknowledgement: Much of this library comes from the 
	"Robotics Primer Workbook" Project hosted on SourceForge.Net
*/
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
void oi_update(oi_t *self)
{
  ...
  for (i = 0; i < 26; i++)
  {
    ...
    if ((i>=12 && i<=15) || (i>=19 && i<=21))
      buf[i] = (signed char)oi_byte_rx();
    else
      buf[i] = oi_byte_rx();
      */
...
void oi_set_speed(int16_t linear_speed, int16_t angular_speed)
{
  ...
  int16_t rightVel = linear_speed + (angular_speed/1e2) * OI_HALF_AXLE_LENGTH;
  int16_t leftVel = linear_speed - (angular_speed/1e2) * OI_HALF_AXLE_LENGTH;
...
//MFT: Load a song
void oi_load_song(int song_index,int num_notes, int *notes,int *duration) {
...
//Requests sensor update and returns distance as cm
int oi_current_distance(oi_t *self) {
	oi_update(self);
  	return((self->dist_m * 100) + (self->dist_mm/10));
