...
void *producer(void *arg)
{
  ...
  while (!producer_done)
  {
    ...
    // Wait for the supply to be 0
    while (supply != 0) {
	    pthread_cond_wait(&producer_cv, &mut);
	  }
	    
	  // We have the mutex lock, so we can just update supply with
	  // the number of elements we are able to produce
		supply = 10;

		// Check to see if the number of consumers remaining is 0
		// If so, set our condition to be done
		// Since we know we aren't creating new consumers, it is safe
		// to check to see if we supplied more than there are consumers
	  if (num_cons_remaining < supply) {
	  	producer_done = 1;
	  }
		
		// Send a broadcast to all waiting threads that there are supplies ready
