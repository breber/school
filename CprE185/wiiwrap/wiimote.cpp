...
// helpers
// ------------------------------------------------------------------------------------
template<class T> __forceinline T sign  (const T& val)  { return (val<0)? T(-1) : T(1); }
...
wiimote::wiimote ()
	...
	// and the state history (for state recording)
...
bool wiimote::Connect (unsigned wiimote_index, bool force_hidwrites)
	...
	// test if HidD_SetOuputReport is supported (XP+ only)
	if(!HidDLL) {
		HidDLL = LoadLibrary(_T("hid.dll"));
		_ASSERT(HidDLL);
		}
	if(HidDLL && !_HidD_SetOutputReport) {
	...
	while(SetupDiEnumDeviceInterfaces(dev_info, NULL, &guid, index, &didata))
		...
		// open a read/write handle to our device using the device path returned
		// ** I've disabled the share flags to easily detect if a particular wiimote
		//    is already in use - this does mean that it must not be opened by
		//    another app either, but that wouldn't be a good idea anway **
		DEEP_TRACE(_T(".. opening device %s"), didetail->DevicePath);
		Handle = CreateFile(didetail->DevicePath, GENERIC_READ|GENERIC_WRITE,
												  0,//FILE_SHARE_READ|FILE_SHARE_WRITE,
												  NULL, OPEN_EXISTING,
												  FILE_FLAG_OVERLAPPED, NULL);
		if(Handle == INVALID_HANDLE_VALUE) {
			DEEP_TRACE(_T(".... failed (may be in use)."));
			goto skip;
			}
		...
		if(HidD_GetAttributes(Handle, &attrib))
			...
			TRACE(_T(".. wiimote installed - trying to write:"));
			...
			if(!bStatusReceived)
				...
				// did we get it?
				if(bStatusReceived)
					TRACE(_T(".. HID write succeeded."));
				else{
					TRACE(_T(".. HID write failed."));
...
void wiimote::Disconnect ()
	...
	Internal.Clear(false); // (preserves deadzones)
	}
// ------------------------------------------------------------------------------------
inline void wiimote::Reset ()
	...
	MuteSpeaker  (true);
	EnableSpeaker(false);
	Sleep(80); // avoids loosing the extension calibration	data on Connect()
	}
// ------------------------------------------------------------------------------------
...
connection_lost:
			...
			remote.Disconnect();
			remote.Internal.Polling.Changes = (state_change_flags)
								(remote.Internal.Polling.Changes | CONNECTION_LOST);
...
int wiimote::ParseInput (BYTE* buff)
	...
	Internal.Polling.Changes = (state_change_flags)(Internal.Polling.Changes | changed);
...
state_change_flags wiimote::_RefreshState (bool for_callbacks)
	...
	if(Internal.Polling.Changes == NO_CHANGE)
		return NO_CHANGE;
	 ...
	 *(wiimote_state*)this = Internal;
	 if(!for_callbacks)
		Internal.Polling.Changes = NO_CHANGE;
...
// ------------------------------------------------------------------------------------
bool wiimote::EstimateOrientationFrom (volatile wiimote_state::acceleration &accel)
	{
...
// ------------------------------------------------------------------------------------
void wiimote::ApplyJoystickDeadZonesAndWrite (volatile wiimote_state::joystick &joy,
											  float new_x, float new_y)
	{
	// apply the deadzones to each axis (if set)
	if((joy.DeadZone.X > 0.f) && (joy.DeadZone.X <= 1.f))
		{
		if(fabs(new_x) <= joy.DeadZone.X)
			new_x = 0;
		else{
			new_x -= joy.DeadZone.X * sign(new_x);
			new_x /= 1.f - joy.DeadZone.X;
			}
		}
	if((joy.DeadZone.Y > 0.f) && (joy.DeadZone.Y <= 1.f))
		{
		if(fabs(new_y) <= joy.DeadZone.Y)
			new_y = 0;
		else{
			new_y -= joy.DeadZone.Y * sign(new_y);
			new_y /= 1.f - joy.DeadZone.Y;
			}
		}
	
	// and write the processed values to the state (this must always happen last so
	//  that no intermediate values are written to the state & possibly read by the app)
	joy.X = new_x;
	joy.Y = new_y;
	}
// ------------------------------------------------------------------------------------
...
int wiimote::ParseIR (BYTE* buff)
	...
	// only update the other values if the dots are visible (so that the last
	//  valid values stay unmodified)
	switch(Internal.IR.Mode)
		{
		case wiimote_state::ir::BASIC:
			Internal.IR.Dot[0].Size = 0;
			Internal.IR.Dot[1].Size = 0;

			Internal.IR.Dot[0].bFound = !(buff[6] == 0xff && buff[ 7] == 0xff);
			Internal.IR.Dot[1].bFound = !(buff[9] == 0xff && buff[10] == 0xff);
			
			if(Internal.IR.Dot[1].bFound) {
				Internal.IR.Dot[1].RawX = buff[ 9] | ((buff[8] >> 0) & 0x03) << 8;
				Internal.IR.Dot[1].RawY = buff[10] | ((buff[8] >> 2) & 0x03) << 8;
				}
			break;
		
		case wiimote_state::ir::EXTENDED:
			Internal.IR.Dot[0].bFound = !(buff[6]==0xff && buff[ 7]==0xff && buff[ 8]==0xff);
			Internal.IR.Dot[1].bFound = !(buff[9]==0xff && buff[10]==0xff && buff[11]==0xff);
		
			if(Internal.IR.Dot[0].bFound)
				Internal.IR.Dot[0].Size = buff[8] & 0x0f;

			if(Internal.IR.Dot[1].bFound) {
				Internal.IR.Dot[1].RawX = buff[ 9] | ((buff[11] >> 4) & 0x03) << 8;
				Internal.IR.Dot[1].RawY = buff[10] | ((buff[11] >> 6) & 0x03) << 8;
				Internal.IR.Dot[1].Size = buff[11] & 0x0f;
				}
			break;
		}

	// in theory the imager is 1024x768 (and so should report 0-1023 x 0-767)
	//  in practice I have never seen the reports exceed the values below,
	//  so I'm using them instead to give the full 0-1 range
	//  (it's possible that the edge pixels are used for processing, or masked
	//  out due to being unreliable)
	static const unsigned MAX_IR_X = 1016;
	static const unsigned MAX_IR_Y = 760;

	if(Internal.IR.Dot[0].bFound) {
		Internal.IR.Dot[0].RawX = buff[6] | ((buff[8] >> 4) & 0x03) << 8;;
		Internal.IR.Dot[0].RawY = buff[7] | ((buff[8] >> 6) & 0x03) << 8;;
		Internal.IR.Dot[0].X    = 1.f - (Internal.IR.Dot[0].RawX / (float)MAX_IR_X);
		Internal.IR.Dot[0].Y    =	    (Internal.IR.Dot[0].RawY / (float)MAX_IR_Y);
#ifdef _DEBUG
		// warn developer to report values outside my assumptions
		if((Internal.IR.Dot[0].RawX > MAX_IR_X) ||
		   (Internal.IR.Dot[0].RawY > MAX_IR_Y))
		   _Message(_T("Wiimote reported raw IR dot0 values larger than expected! :\n")
					_T("\n")
					_T("  X: %d   Y: %d\n")
					_T("\n")
					_T("Please report these values so I can update the lib:\n")
					_T(" glATr-i-lDOTnet\n"), Internal.IR.Dot[0].RawX,
											  Internal.IR.Dot[0].RawY);
#endif // _DEBUG
		}

	if(Internal.IR.Dot[1].bFound) {
		Internal.IR.Dot[1].X	= 1.f - (Internal.IR.Dot[1].RawX / (float)MAX_IR_X);
		Internal.IR.Dot[1].Y =	        (Internal.IR.Dot[1].RawY / (float)MAX_IR_Y);

#ifdef _DEBUG
		// warn developer to report values outside my assumptions
		if((Internal.IR.Dot[1].RawX > MAX_IR_X) ||
		   (Internal.IR.Dot[1].RawY > MAX_IR_Y))
		   _Message(_T("Wiimote reported raw IR dot1 values larger than expected! :\n")
					_T("\n")
					_T("  X: %d   Y: %d\n")
					_T("\n")
					_T("Please report these values so I can update the lib:\n")
					_T(" glATr-i-lDOTnet\n"), Internal.IR.Dot[1].RawX,
											  Internal.IR.Dot[1].RawY);
#endif // _DEBUG
		}
...
int wiimote::ParseExtension (BYTE *buff, unsigned offset)
	...
	switch(Internal.ExtensionType)
		...
		case wiimote_state::NUNCHUK:
			...
			// acceleration
			volatile wiimote_state::acceleration &accel = Internal.Nunchuk.Acceleration;
			...
			accel.X = ((float)accel.RawX - Internal.Nunchuk.CalibrationInfo.X0) / 
					  ((float)Internal.Nunchuk.CalibrationInfo.XG -
							  Internal.Nunchuk.CalibrationInfo.X0);
			accel.Y = ((float)accel.RawY - Internal.Nunchuk.CalibrationInfo.Y0) /
					  ((float)Internal.Nunchuk.CalibrationInfo.YG -
							  Internal.Nunchuk.CalibrationInfo.Y0);
			accel.Z = ((float)accel.RawZ - Internal.Nunchuk.CalibrationInfo.Z0) /
					  ((float)Internal.Nunchuk.CalibrationInfo.ZG -
							  Internal.Nunchuk.CalibrationInfo.Z0);
			...
			// joystick:
			volatile wiimote_state::joystick &joy = Internal.Nunchuk.Joystick;

			joy.RawX = buff[offset+0];
			joy.RawY = buff[offset+1];

			float x,y;
			if(Internal.Nunchuk.CalibrationInfo.MaxX != 0x00)
				x = ((float)joy.RawX - Internal.Nunchuk.CalibrationInfo.MidX) / 
					((float)Internal.Nunchuk.CalibrationInfo.MaxX -
							Internal.Nunchuk.CalibrationInfo.MinX);

			if(Internal.Nunchuk.CalibrationInfo.MaxY != 0x00)
				y = ((float)joy.RawY - Internal.Nunchuk.CalibrationInfo.MidY) / 
					((float)Internal.Nunchuk.CalibrationInfo.MaxY -
							Internal.Nunchuk.CalibrationInfo.MinY);
			...
			float curr_x = Internal.Nunchuk.Joystick.X;
			float curr_y = Internal.Nunchuk.Joystick.Y;

			ApplyJoystickDeadZonesAndWrite(joy, x, y);
	
			if((Internal.Nunchuk.Joystick.X != curr_x) ||
			   (Internal.Nunchuk.Joystick.Y != curr_y))
		...
		case wiimote_state::CLASSIC:
			...
			joyL.RawX = (float) (buff[offset+0] & 0x3f);
			joyL.RawY = (float) (buff[offset+1] & 0x3f);
			joyR.RawX = (float)((buff[offset+2]			>> 7) |
							   ((buff[offset+1] & 0xc0) >> 5) |
							   ((buff[offset+0] & 0xc0) >> 3));
			joyR.RawY = (float) (buff[offset+2] & 0x1f);
	...
	switch(address)
		...
		case (REGISTER_EXTENSION_CALIBRATION & 0xffff):
			...
			switch(Internal.ExtensionType)
				...
				case wiimote_state::NUNCHUK:
					{
					Internal.Nunchuk.CalibrationInfo.X0   = buff[0];
					Internal.Nunchuk.CalibrationInfo.Y0   = buff[1];
					Internal.Nunchuk.CalibrationInfo.Z0   = buff[2];
					Internal.Nunchuk.CalibrationInfo.XG   = buff[4];
					Internal.Nunchuk.CalibrationInfo.YG   = buff[5];
					Internal.Nunchuk.CalibrationInfo.ZG   = buff[6];
					Internal.Nunchuk.CalibrationInfo.MaxX = buff[8];
					Internal.Nunchuk.CalibrationInfo.MinX = buff[9];
					Internal.Nunchuk.CalibrationInfo.MidX = buff[10];
					Internal.Nunchuk.CalibrationInfo.MaxY = buff[11];
					Internal.Nunchuk.CalibrationInfo.MinY = buff[12];
					Internal.Nunchuk.CalibrationInfo.MidY = buff[13];
				...
				case wiimote_state::CLASSIC:
					{
					Internal.ClassicController.CalibrationInfo.MaxXL = buff[ 0] >> 2;
					Internal.ClassicController.CalibrationInfo.MinXL = buff[ 1] >> 2;
					Internal.ClassicController.CalibrationInfo.MidXL = buff[ 2] >> 2;
					Internal.ClassicController.CalibrationInfo.MaxYL = buff[ 3] >> 2;
					Internal.ClassicController.CalibrationInfo.MinYL = buff[ 4] >> 2;
					Internal.ClassicController.CalibrationInfo.MidYL = buff[ 5] >> 2;
					Internal.ClassicController.CalibrationInfo.MaxXR = buff[ 6] >> 3;
					Internal.ClassicController.CalibrationInfo.MinXR = buff[ 7] >> 3;
					Internal.ClassicController.CalibrationInfo.MidXR = buff[ 8] >> 3;
					Internal.ClassicController.CalibrationInfo.MaxYR = buff[ 9] >> 3;
					Internal.ClassicController.CalibrationInfo.MinYR = buff[10] >> 3;
					Internal.ClassicController.CalibrationInfo.MidYR = buff[11] >> 3;
					// this doesn't seem right...
					//	Internal.ClassicController.CalibrationInfo.MinTriggerL = buff[12] >> 3;
					//	Internal.ClassicController.CalibrationInfo.MaxTriggerL = buff[14] >> 3;
					//	Internal.ClassicController.CalibrationInfo.MinTriggerR = buff[13] >> 3;
					//	Internal.ClassicController.CalibrationInfo.MaxTriggerR = buff[15] >> 3;
...
bool wiimote::Load16bitMonoSampleWAV (const TCHAR* filepath, wiimote_sample &out)
	...
	while(1)
		...
		if(!strncmp(chunk_header.ckID, "fmt ", 4))
			...
			// and convert them
			bool res = Convert16bitMonoSamples(samples, true, total_samples,
											  freq, out);
			delete[] samples;
			return res;
			}
