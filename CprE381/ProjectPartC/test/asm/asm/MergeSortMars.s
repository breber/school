...
# FOR USE IN MARS (because it places values in datamem where
# Mars wants it, and then grabs it from there). See MergeSort.s
# for the version to run on our processor

.data 
arr: 20, 15, 35, 1, 70, 63, 124, 3, 15

.text
la $t0, arr
add $s0, $zero, $t0	# $s0 holds the starting address of the array
addi $s1, $zero, 9	# $s1 holds the number of elements in the array
...
MergeSort:
	...
	beqz $t1, SetUpRecursion	# if (b - a not < 2), go to SetUpRecursion
	jr $ra				# b - a is < 2, so return
